...
export default function ProjectProgress({ projectId, role }) {
    ...
    return (
        <div className="fade-up-2">
            ...
            {canEdit && (projectId && projectId !== "all" || editingId) && (
                <form id="progress-form" onSubmit={handleSubmit} className="glass-card" style={{ padding: "24px", marginBottom: "24px" }}>
                        ...
                        <div key={update.id} className="glass-card" style={{ padding: "20px", display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "16px" }}>
                            <div style={{ flex: 1 }}>
                                <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "8px", flexWrap: "wrap" }}>
                                    ...
                                    {update.user?.name && (
                                        <span style={{ fontSize: "12px", color: "var(--text-muted)", background: "rgba(0,0,0,0.03)", padding: "2px 8px", borderRadius: "12px" }}>
                                            👤 {update.user.role?.replace("_", " ")}: {update.user.name}
                                        </span>
                                    )}
                                    {update.project?.name && (!projectId || projectId === "all") && (
                                        <span className="role-badge" style={{ fontSize: "11px", height: "auto", padding: "2px 8px" }}>
                                            {update.project.name}
                                        </span>
                                    )}
                                </div>
                            ...
                            {update.is_archived && (
                                <span style={{ fontSize: "11px", fontWeight: 600, color: "var(--text-muted)", background: "rgba(0,0,0,0.05)", padding: "4px 10px", borderRadius: "6px", textTransform: "uppercase", letterSpacing: "0.02em" }}>
                                    📦 Archived
                                </span>
                            )}
                        </div>
